// Package discord implements the Discord webhook notification provider
// (SPEC §18.5). It POSTs a JSON message to a Discord channel webhook URL.
package discord
...
// Provider is the Discord notification provider.
...
// New returns a Discord provider ready to send.
func New() *Provider { return &Provider{client: &http.Client{}} }

// Kind implements notify.Provider.
func (p *Provider) Kind() string { return "discord" }

// DisplayName implements notify.Provider.
func (p *Provider) DisplayName() string { return "Discord" }

// Fields describes the Discord config form (SPEC §18.5). The webhook URL
// carries the channel's secret token and is therefore a secret field.
...
// Send posts the rendered message to the Discord webhook. Discord's "content"
// field is the only one MVP populates.
func (p *Provider) Send(ctx context.Context, raw json.RawMessage, msg notify.Message) error {
	...
	body, err := json.Marshal(payload{Content: messageText(msg)})
	if err != nil {
...
// payload is the minimal Discord webhook-execute body (SPEC §18.5).
type payload struct {
...
func parseConfig(raw json.RawMessage) (config, error) {
	...
	if err := json.Unmarshal(raw, &cfg); err != nil {
		return cfg, fmt.Errorf("discord: invalid config json: %w", err)
	}
	return cfg, nil
}
